...
def get_bus_routes():
    routes = set()
    with open(os.path.join("source_data", "routes.txt"), encoding="utf-8") as f:
...
def get_trips_by_routes(routes):
    ...
    with open(os.path.join("source_data", "trips.txt"), encoding="utf-8") as f:
        f_csv = csv.DictReader(f)
        for row in f_csv:
...
def get_trips_with_stops(trips):
    ...
    with open(os.path.join("source_data", "stop_times.txt"), encoding="utf-8") as f:
        f_csv = csv.DictReader(f)
        for row in f_csv:
...
def get_stops(trips):
    ...
    with open(os.path.join("source_data", "stops.txt"), encoding="utf-8") as f:
        f_csv = csv.DictReader(f)
        for row in f_csv:
...
def write_csv(graph, stops):
    with open(os.path.join( "data", "graph.csv"), encoding="utf-8",
              mode="w+", newline='') as f:
        f_csv = csv.writer(f)
    ...
    for stop_id in stops:
        stops_list.append([int(stop_id), stops[stop_id]["lat"],
                           stops[stop_id]["lon"]])
    with open(os.path.join("data", "stops.csv"), encoding="utf-8",
              mode="w+", newline='') as f:
        f_csv = csv.writer(f)
